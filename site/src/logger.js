...
export const CSV_FIELDS = [
  ...
  "screen_calibration_status",
  "screen_calibration_protocol",
  "screen_calibration_version",
  "screen_calibration_country_code",
  "screen_calibration_country_name",
  "screen_calibration_coin_id",
  "screen_calibration_currency",
  "screen_calibration_denomination",
  "screen_calibration_diameter_mm",
  "screen_calibration_css_pixels",
  "screen_mm_per_css_pixel",
  "calibrated_viewport_width_mm",
  "calibrated_viewport_height_mm",
  "screen_calibration_repeatability_percent",
  "screen_calibration_timestamp",
  "current_x",
  "current_y",
  "target_x",
  "target_y",
  ...
  "polar_valence_minimum",
  "polar_valence_maximum",
  "polar_valence_inverted",
  "polar_arousal_metric",
  "polar_arousal_value",
  "polar_arousal_normalized",
  "polar_arousal_minimum",
  "polar_arousal_maximum",
  "polar_arousal_inverted",
  "touch_feedback_mode",
  "cursor_hidden",
  "algorithm_version",
  "active_elapsed_ms",
...
function createRecord({ sessionId, sequence, startedAt, now, wallClock, context, recordType, details, state }) {
  ...
  return {
    ...
    screen_calibration_status: recordContext.screenCalibrationStatus ?? "missing",
    screen_calibration_protocol: recordContext.screenCalibrationProtocol ?? "",
    screen_calibration_version: recordContext.screenCalibrationVersion ?? "",
    screen_calibration_country_code: recordContext.screenCalibrationCountryCode ?? "",
    screen_calibration_country_name: recordContext.screenCalibrationCountryName ?? "",
    screen_calibration_coin_id: recordContext.screenCalibrationCoinId ?? "",
    screen_calibration_currency: recordContext.screenCalibrationCurrency ?? "",
    screen_calibration_denomination: recordContext.screenCalibrationDenomination ?? "",
    screen_calibration_diameter_mm: recordContext.screenCalibrationDiameterMm ?? "",
    screen_calibration_css_pixels: recordContext.screenCalibrationCssPixels ?? "",
    screen_mm_per_css_pixel: recordContext.screenMmPerCssPixel ?? "",
    calibrated_viewport_width_mm: recordContext.calibratedViewportWidthMm ?? "",
    calibrated_viewport_height_mm: recordContext.calibratedViewportHeightMm ?? "",
    screen_calibration_repeatability_percent: recordContext.screenCalibrationRepeatabilityPercent ?? "",
    screen_calibration_timestamp: recordContext.screenCalibrationTimestamp ?? "",
    ...
    input_source: state.touchDriveActive
      ? "touch-trace"
      : (state.polarDriveActive ? "polar-stream" : "manual"),
    polar_connected: Boolean(state.polarConnected),
    polar_drive_active: Boolean(state.polarDriveActive),
    polar_valence_metric: state.polarMappings?.valence?.metric ?? "manual",
    polar_valence_value: state.polarAxisValues?.valence?.value ?? "",
    polar_valence_normalized: state.polarAxisValues?.valence?.normalized ?? "",
    polar_valence_minimum: state.polarMappings?.valence?.metric === "manual" ? "" : state.polarMappings?.valence?.minimum ?? "",
    polar_valence_maximum: state.polarMappings?.valence?.metric === "manual" ? "" : state.polarMappings?.valence?.maximum ?? "",
    polar_valence_inverted: state.polarMappings?.valence?.metric === "manual" ? "" : Boolean(state.polarMappings?.valence?.invert),
    polar_arousal_metric: state.polarMappings?.arousal?.metric ?? "manual",
    polar_arousal_value: state.polarAxisValues?.arousal?.value ?? "",
    polar_arousal_normalized: state.polarAxisValues?.arousal?.normalized ?? "",
    polar_arousal_minimum: state.polarMappings?.arousal?.metric === "manual" ? "" : state.polarMappings?.arousal?.minimum ?? "",
    polar_arousal_maximum: state.polarMappings?.arousal?.metric === "manual" ? "" : state.polarMappings?.arousal?.maximum ?? "",
    polar_arousal_inverted: state.polarMappings?.arousal?.metric === "manual" ? "" : Boolean(state.polarMappings?.arousal?.invert),
    touch_feedback_mode: state.touchFeedbackMode ?? "gated",
    cursor_hidden: Boolean(state.touchDriveActive && state.touchHideCursor),
