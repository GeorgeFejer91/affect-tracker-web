...
test("remote pages load only the local SDK and feature code makes no microphone or audio request", () => {
  ...
  assert.match(index, /src="\.\/src\/app\.js\?v=remote-2"/);
  assert.match(webxr, /src="\.\/src\/webxr-study\.js\?v=remote-2"/);
  assert.match(app, /from "\.\/flubber-remote\.js\?v=remote-2"/);
  assert.match(receiver, /from "\.\/flubber-remote\.js\?v=remote-2"/);
