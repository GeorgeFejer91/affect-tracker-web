...
- Delivered as an independent experimental browser-to-browser settings slice: explicit public broadcast/discovery of one immutable complete portable-settings v1 snapshot, reliable ordered VDO data transfer, one/many-source selection, exact size/schema/source validation, formatted preview, and a separate Apply gesture. It has no X/Y stream, heartbeat, project backend, persistence, or automatic page-load connection.
