...
- Delivered: touch-first GitHub Pages smartphone viewer with one-time Affect-controller discovery, compact safe-area-aware navigation, an equal split between the canonical live Flubber and a large direct 2D valence/arousal field, grab-existing-marker-only coordinate drag semantics, a separately draggable upper Flubber whose normalized movement can synchronize across differently sized browser viewports, and portrait/landscape phone layouts. The experimental movement-analysis Touch Lab remains separately available. This remains a browser experience, not a native smartphone app.
