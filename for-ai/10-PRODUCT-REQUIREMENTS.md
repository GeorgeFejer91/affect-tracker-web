...
- Optional `vr.flubber.controllerFollow` rigging defaults off, uses the left Touch controller by default, places the panel a configurable `0.05–0.6 m` beyond that tracked controller on the wearer-to-controller ray, and recomputes a full viewer-facing orientation from live head/controller poses every frame. When enabled, this tracking authority supersedes world dragging and A-button recentering; both retain their existing behavior when it is disabled. Lost tracking leaves the last safe pose in place and emits one state edge rather than hiding the instrument.
...
- `vr.environment` accepts `dark` or `passthrough`. The web exporter's mixed-reality checkbox selects compositor-owned Quest passthrough, allowing flat video and the transparent Flubber to appear over the wearer's normal see-through view. The APK does not request camera frames, record passthrough, or place passthrough pixels in JSON/LSL; immersive 180°/360° carriers may naturally occlude the background where they render. Hand tracking remains out of scope for v1.
