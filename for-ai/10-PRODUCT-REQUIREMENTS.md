...
- The playground exposes a browser-local feedback behavior independent of `inputSource`: **Gated occasional swipes** is the default and **Continuous live response** remains available for comparison. A gated movement window closes after 400 ms of inactivity, contributes once to participant calibration, and applies a persistent signed nudge: slow/fast lowers/raises arousal and jagged/round lowers/raises valence. A 0.12 normalized-evidence dead zone suppresses ambiguous windows and a strong gate is capped at 0.25 per axis, so repeated occasional gestures accumulate to `[-1,1]`. Gated values do not decay; another gate or Reset changes them. Continuous behavior retains the 1.8-second hold and 3-second release.
