...
`site/src/touch-trace.js` is a dependency-free browser-only signal layer. It owns the 1€ position filters, equal-distance resampling, turn metrics, bounded rolling histograms, asymmetric range smoothing, confidence gating, intermittent gate commits, continuous-mode inactivity decay, and trace-fit geometry. `app.js` owns Pointer Events acquisition, UI exclusion, rendering, experiment lifecycle, and logging. The portable settings schema remains version 1; `inputSource`, touch feedback behavior, trace visibility, and cursor hiding are separate browser-local preferences.
...
The default gated feedback path groups delivered movement into a window that closes after 400 ms without a valid point. Feature snapshots inside the gate are bounded, but only the gate representative is added to each adaptive range, giving short and long gestures equal calibration weight. Gated calibration retains up to 120 completed windows and blends from priors over 20 qualified gates. At commit, confidence-weighted shape and speed evidence pass through a 0.12 dead zone and a signed `0.05..0.25` step mapping; the resulting delta accumulates against the existing touch target and remains there. The optional continuous path retains 1,200 feature samples, the 100-sample bootstrap, 300 ms attack, 1.8-second hold, and 3-second neutral release. Switching behavior resets analyzer calibration but seeds the new behavior from the existing affect target.
