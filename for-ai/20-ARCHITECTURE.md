...
`site/src/vr-session.js` owns the additive Quest envelope and exporter. It nests a normalized portable settings v1 object without extending that schema. Pixel overlay position/size remain round-trippable web/desktop data but are not Quest placement authority; `vr.flubber` owns metric placement.
