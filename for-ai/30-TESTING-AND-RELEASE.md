...
- Multi-video acceptance includes: the active manifest remains selected by default; selection survives Ready-screen rescans; only fully validated optional manifests appear as choices; optional copy-in-progress/hash/layout/session-ID failures are visible but do not block the active session; and the immersive runtime receives the selected staged fingerprint. A physical headset playback check is required for at least one flat stereo sample and one mono equirectangular 360° sample.
