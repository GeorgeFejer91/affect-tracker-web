...
- Verify gated behavior is the default, closes one window after 400 ms of inactivity, applies slow/fast as negative/positive arousal and jagged/round as negative/positive valence, suppresses evidence through 0.12, caps each signed axis step at 0.25, accumulates/clamps repeated gestures, persists without decay, and clears on Reset. Short and long gates must each contribute exactly one adaptive sample, gated calibration must blend over 20 qualified windows with 120-window bounded storage, and experiment finalization must commit an open terminal gate before the final metric/sample.
...
- Verify `pointer_raw`, `touch_metric`, `sample`, and `event` rows are chronological and distinguishable; `speed_continuity_active` records the micro-stroke carry decision; touch feedback mode, gate identity/open state/duration/commit sequence/deltas, and calibration counts are reconstructable; each gate emits one `gate-commit` event; append-only experiment storage does not roll over; active time pauses during buffering; final/partial export and retry retain all rows; long-trial warnings remain non-blocking.
