...
- Run web manifest tests and Kotlin renderer/engine/strict-parser tests before any APK or headset step. Verify omitted `showAffectValues` remains false, explicit booleans round-trip, invalid types fail closed, the default stick is right, and the optional readout formats current/target/rate/stick values with locale-stable decimal points at no more than 10 Hz.
