...
The data-only transport in the deployed `remote-13` runtime is now physically qualified on the available Quest for deterministic synthetic ECG-derived X/Y streaming over both Direct P2P and forced TURN. Current-build immersive cadence, route, loss/recovery, latest-state backpressure, and browser-wire latency receipts are recorded below. `3c8df81` is still **not fully physically qualified for worn-H10 research use** because the H10 had been removed; the browser recovery path remains deterministic/mock hardware evidence rather than a worn-sensor receipt.

An earlier physical direct-path receipt on commit `8d8c813` remains useful historical evidence because the subsequent runtime changes added explicit forced-TURN qualification, foreground lifecycle/recovery, ideal-deadline sender scheduling, teardown and channel-close hardening, browser-ECG liveness recovery, and cache revisioning without changing the wire codec. It is not substituted for the current-build receipts below.
...
## Current physical Quest synthetic qualification (`remote-13`)

After USB debugging became authorized, the deployed public `remote-13` pages were loaded fresh in a dedicated desktop Chrome 151 profile and the attached Meta Quest Browser. Both pages exposed the exact current cache revisions before either explicit remote button was pressed. The desktop used deterministic 130 Hz synthetic ECG with Heart rate mapped to X and Local ECG power mapped to Y; no Bluetooth device, real physiology, or participant identifier was used. The sender retained its floating foreground Flubber window and wake lock. The receiver entered immersive WebXR before each accepted soak.

| Physical route | Stable interval | Effective receive cadence | Independent route evidence | Receiver timing and stability |
| --- | --- | --- | --- | --- |
| Direct P2P | 122.123 s; 7,234 new accepted frames | 59.24 frames/s | Sender Direct P2P; 4–8 ms WebRTC RTT | 19–22 ms rolling p95 packet gap; 99–100 ms retained maximum; 0 loss warnings |
| Forced TURN | 123.084 s; 7,242 new accepted frames | 58.84 frames/s | Both endpoints reported TURN relay; sender 37–41 ms RTT | 22–27 ms rolling p95 packet gap; 92–125 ms retained maximum; 0 loss warnings |

The direct source was then paused without changing its selected source or channel. A measured 1.001 s pause increased the retained gap to 1,093 ms but produced no warning. During a measured 3.5 s pause the Quest held its last coordinates and exposed one `signal lost; holding` warning; after resume, three consecutive accepted frames restored live state automatically, the retained gap became 3,557 ms, and the warning count remained exactly one. No live/lost flapping was observed.

The forced-TURN sender was also subjected to page-local injected backpressure. Its `bufferedAmount` readback remained nonzero long enough to discard 133 offers instead of queuing them. The receiver held the last state, recorded one warning when the injected interval crossed the two-second grace, and returned to live on the current coordinates after pressure cleared. No obsolete sequence drained afterward. This is an injected congestion-policy receipt, not a claim about naturally occurring Wi-Fi saturation.

The latency probes kept the 12-byte production packet unchanged and recorded temporary in-memory diagnostics only:

- In the most representative continuous Direct P2P run, wrappers at `RTCDataChannel.send` and the Quest data-channel message dispatch captured 1,315 sequence-matched frames while immersive. Send-to-receive latency was 3.37 ms median, 7.57 ms p95, and 69.47 ms maximum. Best-path CDP clock calibration contributed approximately 0.54 ms combined half-RTT uncertainty. Twelve of 1,327 in-scope sent sequences were absent at the receiver (approximately 0.9%), consistent with the intentional unordered, zero-retry newest-state policy; there was no stale transition or backlog.
- A 120-step direct sender probe measured the source-event-to-send scheduler at 10.40 ms median, 15.40 ms p95, and 17.60 ms maximum, confirming the expected one-60-Hz-frame ceiling. Sparse/alternating updates showed a longer Quest scheduling tail—14.80 ms median, 102.70 ms p95, and 149.10 ms maximum from injected input to receiver dispatch—while the continuously changing mapped source above retained its 7.57 ms wire p95. This distinction is retained rather than presenting only the best path.
- A separate 30-step forced-TURN browser probe measured 31.76 ms median, 48.96 ms p95, and 53.56 ms maximum from injected desktop coordinate change to Quest browser value receipt. A clock-independent host-observed upper bound was 32.59 ms median, 50.53 ms p95, and 54.65 ms maximum. The test independently retained TURN readback at approximately 37–41 ms RTT.

These measurements are browser-software receipts, not optical motion-to-photon measurements. On the direct continuously changing path, an engineering scheduling budget is approximately one 60 Hz sender frame (at most 16.67 ms), the measured 7.57 ms wire p95, and one 90 Hz Quest XR frame (at most 11.11 ms), or about 35 ms before unmeasured display scanout. That sum is a component bound, not a measured end-to-end percentile. A synchronized LED/photodiode or high-speed-camera test would still be required for a defensible motion-to-photon number. The ordinary ADB screencapture route returned a black immersive surface plus the performance overlay, so it is treated as a capture-route limitation and not as visual proof of rendered Flubber pixels; app-owned WebXR state and packet diagnostics provide the receipt.
...
The wire deliberately carries no timestamp, so production packets alone cannot yield one-way latency. The current physical qualification kept that privacy-minimal packet intact and used temporary send/receive instrumentation plus best-path clock calibration to measure browser-wire latency. The sender adds at most one 60 Hz scheduling interval when it is foregrounded, and the receiver applies an accepted coordinate pair on its next XR frame without a second smoothing or jitter buffer. Neither the instrumented browser receipt nor those scheduling bounds substitute for a synchronized optical motion-to-photon measurement.

## Remaining physical gate

The synthetic desktop-to-Quest transport matrix is complete for deployed `3c8df81`. The remaining physical gate is the worn sensor: with a worn H10 and no competing Polar application or tab, repeat Connect, the acknowledged-but-silent setup recovery case where reproducible, at least two minutes of live ECG through the same Direct P2P headset path, explicit disconnect, and reconnect. An optical motion-to-photon test remains optional follow-up if the study requires a display-level latency claim rather than the browser-software receipt above.

## Cleanup

All public receivers and broadcasters used for the direct, TURN, foreground-loss, reselection, and current physical qualification checks were explicitly stopped. The final synthetic replay was explicitly disconnected. The dedicated qualification Chrome profile was closed and moved to the Recycle Bin without affecting the user's ordinary Chrome profile. The temporary CDP helpers, raw captures, and probe scripts were also moved to the Recycle Bin. The single temporary Quest DevTools ADB forward was removed and the owned host ports were verified closed; the shared ADB daemon and headset power/performance settings were left untouched. Nine privacy-safe historical diagnostic screenshots remain committed under `for-ai/evidence/remote-flubber-2026-08-25/`; no physiology, Bluetooth identifier, participant identifier, or Quest serial appears in the report or committed evidence.
