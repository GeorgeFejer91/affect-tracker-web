...
Rust is the only desktop authority for affect state. Each compact snapshot supplies the exact same `currentX`, `currentY`, and `phase` object to both settings-window renderers, so facial deformation and the Flubber's shape, pulse, and rate of change move together. The presentation selector offers AFFEC empirical 3D, MediaPipe-rigged atlas 3D, continuous FACS-style 3D, a 21 × 21 matrix-anchor 3D profile, and a photoreal atlas blend. The matrix-anchor face profile uses a compact 441-record coefficient cache at exact `0.1` nodes and evaluates the same anchor surface continuously for off-grid Rust snapshots; it is presentation only and is not the traversal authority. The first four modes deform the same locally packaged morph head; the final mode continuously blends a compact project-owned 11 × 11 atlas whose cells were landmark-warped offline from nine owned anchors. No camera or recognition model runs in the app. These are non-diagnostic coordinate visualizations, and reduced motion never changes the selected coordinates.

Two separate Rust-owned session traversal modes remain available:
