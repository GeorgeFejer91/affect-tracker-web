...
5. Select the local video, declare projection/stereo, configure placement/controllers, optionally check **Show numerical affect values**, and export. New sessions select the right thumbstick by default; either hand remains configurable.
...
8. Put on the headset, look comfortably forward, choose a validated experiment, optionally change **Show live affect values** for this run, then press **Start experiment**. The headset switch starts from the JSON value but is a transient per-run override; it does not rewrite the manifest. LSL opens immediately; the flat screen and alpha-blended Flubber recenter from the live head pose, Flubber appears during a 3-second countdown, and video begins afterward. The mapped thumbstick remains active whenever the visible Flubber exists, including countdown and pause. Hold either trigger anywhere on the transparent Flubber square—even an empty corner—to move it laterally or in depth.
