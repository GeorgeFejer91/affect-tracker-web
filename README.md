...
The GitHub Pages app keeps an affect face on the main stage to the left of the canonical Flubber. The top-level **Synchronized Face + Flubber** accordion enables, disables, centers, and selects that presentation; closing the accordion does not remove the face. On phones, the live **Face** view exposes a compact face-appearance selector in its header, so all six engines can be changed without opening **Face options**. When either Photoatlas mapping is selected, a second compact **Portrait** selector appears beside it; the detailed Face options and desktop settings window expose the same choice. These controls write the same browser-local presentation state and reuse the existing renderer pair. Every mode receives the exact same displayed valence, arousal, and animation-phase snapshot as its Flubber and owns no input or research clock.

The selectable main-tracker modes are **AFFEC empirical 3D**, which uses aggregate perceived-rating locations from 5,807 valid AFFEC observations to blend project-authored expression prototypes on a locally packaged Vitruvian morph head; **AFFEC-guided Photoatlas**, which uses those same aggregate locations to guide a separately disclosed project-authored mapping into the selected portrait atlas; **MediaPipe-rigged atlas 3D**, which uses compact blendshape signals extracted once at build time from nine project-owned atlas cells; **Continuous FACS-style 3D**, a project-authored continuous morph mapping; **21 × 21 matrix-anchor 3D**, which keeps one compact coefficient record for each of the 441 exact `0.1`-spaced nodes and evaluates the same anchor surface continuously for an off-grid coordinate; and **Direct-grid Photoatlas**, which maps tracker coordinates directly to the atlas for comparison. The AFFEC evidence aggregate and the authoring binding are separate hash-bound files: AFFEC supports the six aggregate perceived-category locations, while the category-to-portrait correspondences, transfer surface, portrait anchors, 441 derived cells, and appearances remain explicitly unvalidated project work. Photoatlas offers the original portrait plus eight separately packaged fictional synthetic presets spanning feminine-coded, masculine-coded, and androgynous presentation styling and a broad range of creator-chosen appearance inspirations and skin tones. Public names stay neutral, and those inspirations are provenance—not claims about any person's gender identity, sex, race, ethnicity, ancestry, or culture. Each preset is its own 21 × 21 atlas: the first seven synthetic packs and the original are generated offline from nine project-owned expression anchors, while experimental `photo-synthetic-08` starts from a newly generated 5 × 5 sheet containing 25 newly generated project-owned image-to-image anchors—16 additional target positions beyond the earlier 3 × 3 reference lattice. Because all 25 cells are new generations, its nine reference-position cells are not claimed to preserve the earlier pixels byte-for-byte. Landmark-guided piecewise-affine warping and premultiplied blending derive the 441 runtime cells, then adjacent cells are interpolated continuously in the browser. Only the selected atlas is requested and decoded. This moves features between frames instead of merely double-exposing distant expressions, retains an exact neutral center, and adds no runtime model. The coefficient cache duplicates no mesh, texture, or image. The detailed modes use locally vendored Three.js and checked-in GLB/textures, with a friendly-soft material treatment that neutralizes the source sclera's strong pink vasculature, remaps the amber iris to muted gray-green, softens excessive red skin chroma, and suppresses wet/glassy highlights. Softer low-specular studio lighting also reduces harsh reflection on skin and mouth without changing geometry or affect weights. The canonical SVG fallback retains neutral-white eyes with dark pupils, and the natural photo pixels are never affect-tinted. Detailed rendering degrades locally through the default local photo atlas to the vector face if needed. The application downloads no face model at runtime, requests no camera, performs no face recognition or demographic inference, and does not claim that a cached visualization uniquely identifies an affective state. RIKEN's published five-by-five valence/arousal study contributes only the grid-design lesson, and 4DFAB contributes only a dense-correspondence/synthesis blueprint; no participant media, annotations, restricted-source model, or restricted-source derived data is used or redistributed. The generated output has not been validated as perceived valence/arousal; blinded human ratings of anchors, intermediate cells, and transition paths remain required. Asset and data provenance are recorded in the [local face notice](./site/assets/affect-face/NOTICE.md) and [research ledger](./for-ai/70-RESEARCH-PROVENANCE.md).

**Face options** on GitHub Pages also selects how the browser-owned affect coordinate moves. **Smooth continuous** retains the existing continuous response. **21 × 21 step matrix** exposes 441 exact states from `-1` to `+1` on both axes in `0.1` increments, including exact neutral at zero-based cell `(10,10)` (displayed as row/column 11). A direct phone-grid tap or drag can target any cell. The current state follows the shortest 8-connected route—diagonal while both axes differ, then cardinal—at `0.5–20` states per second. **Stop** holds the current node and **Go to neutral** targets exact `(0,0)`. Although these controls are presented in Face options, the traversal belongs to the canonical browser affect state: face and Flubber consume the same frozen current-X/current-Y/phase snapshot at every node. It is session-only and does not change portable settings, LSL, WebXR, native Quest, or the separate desktop traversal described below.
...
The GitHub Pages application has a touch-first phone layout; no native smartphone app is required. On the first visit from a narrow touch-capable device, **Affect** opens the direct controller without enabling Experimental Touch/Trackpad tracking. Affect and Face reuse one live controller with the face-left/Flubber-right preview above the 2D valence–arousal grid. Face opens to that preview with an inline **Face** selector for immediate switching among all six renderers; **Face options** retains visibility, transition, rate, centering, detailed descriptions, and disclosures. In smooth mode the existing marker must be grabbed before direct movement. In 21 × 21 step-matrix mode, a tap or drag anywhere on the phone grid selects the nearest exact target cell. Compact protocol tabs, safe-area/notch support, dynamic viewport height, and the separate non-scrolling Touch Lab remain available. Portrait phones up to 600 CSS px wide and coarse-pointer phone landscape viewports up to 500 CSS px tall receive the compact layout.
