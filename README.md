...
The Experiment panel also offers optional physical screen calibration. Select one of 30 round coins from 30 currencies/regions, enter fullscreen, place the real coin on the display, and resize the square until the coin touches the midpoint of all four sides while the corners remain visible. Two fits must agree within 3%. A valid result is stored only in this browser and adds the reference coin, scale, repeatability, and calibrated fullscreen viewport dimensions to experiment CSV; it becomes stale after a screen-size, pixel-ratio, or orientation change. This physical reference is necessary because browser CSS units alone do not reliably reveal a display's manufactured size.
