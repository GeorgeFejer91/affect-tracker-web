...
The GitHub Pages application has a touch-first phone layout; no native smartphone app is required. On the first visit from a narrow touch-capable device, **Affect** opens the direct controller without enabling Experimental Touch/Trackpad tracking. Affect and Face reuse one live controller with the face-left/Flubber-right preview above the 2D valence–arousal grid. Face opens to that preview with an inline **Face** selector for immediate switching among all five renderers; **Face options** retains visibility, transition, rate, centering, detailed descriptions, and disclosures. In smooth mode the existing marker must be grabbed before direct movement. In 21 × 21 step-matrix mode, a tap or drag anywhere on the phone grid selects the nearest exact target cell. Compact protocol tabs, safe-area/notch support, dynamic viewport height, and the separate non-scrolling Touch Lab remain available. Portrait phones up to 600 CSS px wide and coarse-pointer phone landscape viewports up to 500 CSS px tall receive the compact layout.
