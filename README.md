...
The GitHub Pages app keeps an affect face on the main stage to the left of the canonical Flubber. The top-level **Synchronized Face + Flubber** accordion enables, disables, centers, and selects that presentation; closing the accordion does not remove the face. On phones, the live **Face** view also exposes a compact face-appearance selector in its header, so all five engines can be changed without opening **Face options**. Both selectors write the same browser-local mode and reuse the same renderer pair; **Face options** remains the detailed control and description surface. The desktop settings window provides the same five choices from its Rust-authoritative snapshot. Every mode receives the exact same displayed valence, arousal, and animation-phase snapshot as its Flubber and owns no input or research clock.

The selectable main-tracker modes are **AFFEC empirical 3D**, which uses aggregate perceived-rating locations from 5,807 valid AFFEC trials to blend project-authored expression prototypes on a locally packaged Vitruvian morph head; **MediaPipe-rigged atlas 3D**, which uses compact blendshape signals extracted once at build time from nine project-owned atlas cells; **Continuous FACS-style 3D**, a project-authored continuous morph mapping; **21 × 21 matrix-anchor 3D**, which keeps one compact coefficient record for each of the 441 exact `0.1`-spaced nodes and evaluates the same anchor surface continuously for an off-grid coordinate; and **Photoreal atlas blend**, an 11 × 11 local project-created synthetic atlas. Its 121 cells are generated offline from the nine owned expression anchors by landmark-guided piecewise-affine warping and premultiplied blending; the browser then interpolates adjacent cells continuously. This moves features between frames instead of merely double-exposing distant expressions, while retaining an exact neutral center and adding no runtime model. The coefficient cache duplicates no mesh, texture, or image. The detailed modes use locally vendored Three.js and checked-in GLB/textures, with a friendly-soft material treatment that neutralizes the source sclera's strong pink vasculature, remaps the amber iris to muted gray-green, softens excessive red skin chroma, and suppresses wet/glassy highlights. Softer low-specular studio lighting also reduces harsh reflection on skin and mouth without changing geometry or affect weights. The canonical SVG fallback retains neutral-white eyes with dark pupils, and the natural photo pixels are never affect-tinted. Detailed rendering degrades locally through that photo atlas to the vector face if needed. The application downloads no face model at runtime, requests no camera, performs no face recognition, and does not claim that a cached visualization uniquely identifies an affective state. Asset and data provenance are recorded in the [local face notice](./site/assets/affect-face/NOTICE.md) and [research ledger](./for-ai/70-RESEARCH-PROVENANCE.md).
