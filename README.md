...
The playground gives immediate practice feedback for detected pointer type, shape, speed, confidence, the last four seconds of movement, and a live valence–arousal color map with a moving dot. Two measured movement segments are enough for full speed confidence. Rapid touch/pen strokes beginning within 900 ms share their recent speed evidence, so repeated short up/down swipes can reach high arousal without joining their separate shapes or counting the lifted-finger jump. The last mapped result is held for 1.8 seconds and then returns gradually toward neutral rather than snapping back. The mode adapts its speed and shape ranges to the current participant with bounded rolling percentiles. **Hide mouse cursor while tracking** removes the pointer over movement/stimulus areas while deliberately leaving it visible over settings so the option is reversible. Enable **Show movement trace beneath Flubber** to mirror the trace in a floating rectangle below the Flubber. Neither rectangle constrains movement. Flubber dragging and manual affect changes are disabled while this source is selected, although physical inputs remain logged during experiments.
