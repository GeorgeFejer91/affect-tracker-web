...
The default **Gated occasional swipes** behavior is designed for intermittent input. A short movement window closes after 400 ms without a new point and applies one lasting nudge: fast/slow moves arousal up/down and round/jagged moves valence right/left. Ambiguous windows inside a dead zone do nothing; stronger windows move by up to `0.25`, so repeated gestures accumulate to either extreme. The Flubber holds the accumulated result until another swipe or Reset. Each completed window contributes only one sample to the participant-specific calibration, preventing a long gesture from outweighing many short gestures. Calibration blends from safe priors over the first 20 qualified windows.
